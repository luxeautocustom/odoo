...
class SaleOrderAuthRefund(models.Model):
    ...
    def default_get(self, fields):
        ...
        if context.get('active_id') and context.get('active_model') == 'sale.order':
            ...
            for transaction in sale_order_id.transaction_ids.sudo().filtered(lambda tx: tx.transaction_type == 'debit' and not tx.echeck_transaction\
                and not tx.refund_amount >= tx.amount and tx.state == 'done' and tx.payment_id and tx.payment_id.authorize_payment_type == 'credit_card'):
    ...
    def auth_refund(self):
        ...
        try:
            ...
            for rec in self.auth_invoice_refund_ids:
                ...
                if rec.acquirer_id and not rec.acquirer_id.journal_id:
                    raise ValidationError(_("Please configure Authorize payment journal"))
                journal_id = rec.acquirer_id and rec.acquirer_id.journal_id
                ...
                if rec.credit_amount and rec.refund_amount and rec.transaction_id and \
                    ...
                    if rec.transaction_id and card_number and rec.payment_transaction_id.acquirer_reference:
                        charge_data.update({
                        ...
                        if resp is not None:
                            ...
                            if refund_tran_id and status_code:
                                tx_vals = {'acquirer_reference': str(refund_tran_id)}
                                ...
                                if status_code == 1:
                                    ...
                                    payment_id = transaction_id.create_payment_vals(trans_id=str(refund_tran_id), authorize_partner=rec, authorize_payment_type='credit_card')
...
class AuthorizeInvoiceRefund(models.Model):
    ...
    def onchange_currency_id(self):
        if self.currency_id:
            from_currency_id = (self.payment_transaction_id and self.payment_transaction_id.currency_id) or self.acquirer_id.journal_id.currency_id
