...
class AccountPaymentRegister(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if context.get('is_register_pay') and context.get('active_id') and context.get('active_model') == 'account.move':
            transaction_ids = []
            ...
            if not cid and company_invoice_id:
                token = invoice_id.partner_id.payment_token_ids.filtered(lambda x: x.company_id.id == invoice_id.company_id.id and\
                            x.acquirer_ref and x.acquirer_id.provider == 'authorize')
            elif not cid:
                token = invoice_id.partner_id.payment_token_ids.filtered(lambda x: x.company_id.id == self.env.company.id and\
                            x.acquirer_ref and x.acquirer_id.provider == 'authorize')
            ...
            if cid and invoice_id:
                res.update({
                    ...
                    'acquirer_id': cid and cid.acquirer_id.id if cid else token.acquirer_id.id,
                })
            if res.get('customer_profile_id') and invoice_id and invoice_id.move_type in ['out_refund', 'in_refund']:
                ...
                res.update({'auth_invoice_refund_ids': [(6, 0, transaction_ids)], 'partner_id': invoice_id.partner_id.id, 'is_refund_ref': is_refund_ref})
        return res
    ...
    authorize_payment_type = fields.Selection([('credit_card', 'Credit Card'), ('bank_account', 'eCheck.Net')],
                                              'Authorize Transaction', copy=False)
    ...
    def onchange_cc_number(self):
        ...
        if self.payment_authorize and invoice_id:
            ...
            if not payment_methods and self.acquirer_id and self.acquirer_id.provider == 'authorize':
            ...
            if from_currency_id and to_currency_id and from_currency_id != to_currency_id:
                self.amount = from_currency_id._convert(self.amount, to_currency_id, self.company_id or self.acquirer_id.journal_id.company_id, fields.Date.today())
        ...
        if self.payment_type == 'inbound' and self.payment_token_id and self.payment_authorize and not self.payment_token_id.authorize_card:
            ...
            if cid:
                self.update({
                    ...
                    'acquirer_id': cid.acquirer_id.id
                })
